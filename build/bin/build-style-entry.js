...
function build(folder, isESModule) {
  ...
  components.forEach(componentName => {
    const content = analyzeDependencies(componentName, dir)
      .map(component => isESModule ? `import '../../klg-css/${component}.css'` : `require('../../klg-css/${component}.css')`);
  ...
  function analyzeDependencies(componentName, dir) {
    ...
    search(dependencyTree({
      ...
      filter: path => path.indexOf(`klg${SEP}${folder}${SEP}`) !== -1
    }), checkList, whiteList);
  ...
  function search(tree, checkList, whiteList) {
    tree && Object.keys(tree).forEach(key => {
      ...
      const component = key.split(`${SEP}klg-admin-pro${SEP}${folder}${SEP}`)[1].replace(`${SEP}index.js`, '').replace(`mixins${SEP}`, '');
  ...
  function checkComponentHasStyle(componentName) {
    return fs.existsSync(path.join(__dirname, `../../${folder}/klg-css/`, `${componentName}.css`));
  }
}
